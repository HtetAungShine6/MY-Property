...
class FavoriteCollectionViewCell: UICollectionViewCell {
    ...
    func configure(with listing: FavoriteListing) {
        listingName.text = listing.listingName
        
        if let price = listing.listingPrice {
            listingPriceRange.text = "\(Int(price.rounded())) THB"
        } else {
            listingPriceRange.text = "Price not available"
        }
        
        if let imageUrlString = listing.listingImage, let imageUrl = URL(string: imageUrlString) {
